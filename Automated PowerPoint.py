...
class CreatePresentation:
    ...
    def __init__(self, input_pptx_filepath=""):
        self.prs = Presentation(input_pptx_filepath) if input_pptx_filepath else Presentation()
    ...
    def _check_slide_shapes(self, slide):
    ...
    def add_slide__Picture_w_Caption(self, picture, title='', text='', print_slide_shapes=False):
        ...
        if print_slide_shapes:
            self._check_slide_shapes(slide)
    ...
    def add_slide__Blank(self, add_picture="", add_textbox="", print_slide_shapes=False):
        layout_name = "Blank"
        slide = self._add_slide(layout_name=layout_name)
        ...
        if add_textbox:
            textbox = slide.shapes.add_textbox(left=Inches(0.5), top=Inches(0.5),
                                               width=self.prs.slide_width-Inches(1),
                                               height=self.prs.slide_height-Inches(1))
            textbox.text_frame.paragraphs[0].text = add_textbox


    def move_slide(self, old_idx, new_idx):
...
if __name__ == "__main__":
    ...
    for sf in sub_folders:
        for i in range(1, 21):
            ...
            presentation = CreatePresentation(os.path.join(folder, "presentations", f"{sf} - {i}.pptx"))
            with open(os.path.join(folder, sf, str(i), "log_timestamps.txt"), "r") as file:
                text = file.read()
            presentation.add_slide__Blank(add_textbox=text)
            ...
            print("Saving pptx")
            presentation.save_pptx(os.path.join(folder, "presentations", f"{sf} - {i}.pptx"), start_file=False)
            print("Done!")
